...
object Control {
  ...
  def main(args: Array[String]) {
    ...
    val token = getToken
    val yearToHour = getYearToHour(args(0).toInt)
    val date = yearToHour.substring(0, 8)
    ...
    val dir = new File(args(1) +"/"+ date)
    if (!dir.exists()) dir.mkdirs()
    val write = new PrintWriter(args(1) +"/"+ date + "/" + yearToHour + ".txt")
...
//    println(url)
  ...
  private def md5Encode(str: String): String = {
    ...
    val hexValue: mutable.StringBuilder = new mutable.StringBuilder
...
//    println(Parameter.SH_KV_URL_HEAD + url)
    ...
    val resultBuilder: mutable.StringBuilder = new mutable.StringBuilder
